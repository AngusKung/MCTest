...
class DNN:
	...
	def init_model(self, pre_train): # default 1 hidden layer
		...
		self.parameters = []
		for i in range(self.n_hid_layer+1):
			if(i == 0):
				w = theano.shared(np.random.uniform(low=-0.1, high=0.1, size=
							(self.input_dim, self.hid_layer_dim)).astype(dtype="float32"))
				b = theano.shared(np.random.uniform(low=-0.1, high=0.1, size=
							(self.hid_layer_dim)).astype(dtype="float32"))
			elif(i == self.n_hid_layer):
				w = theano.shared(np.random.uniform(low=-0.1, high=0.1, size=
							(self.hid_layer_dim, self.output_dim)).astype(dtype="float32"))
				b = theano.shared(np.random.uniform(low=-0.1, high=0.1, size=
							(self.output_dim)).astype(dtype="float32"))
			else:	
				w = theano.shared(np.random.uniform(low=-0.1, high=0.1, size=
							(self.hid_layer_dim, self.hid_layer_dim)).astype(dtype="float32"))
				b = theano.shared(np.random.uniform(low=-0.1, high=0.1, size=
							(self.hid_layer_dim)).astype(dtype="float32"))
